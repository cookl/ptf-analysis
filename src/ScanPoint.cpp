...
ScanPoint::ScanPoint(){
  ...
  fTime_1=0;
  fT_int1=0;fT_ext1=0;fT_ext2=0;
}

ScanPoint::ScanPoint( double x, double y, double z,double time_1,double t_int1,double t_ext1,double t_ext2, unsigned long long entry, unsigned long long entries ){
  fX = x;  fY = y;  fZ = z;
  fEntry = entry;
  fEntries = entries;
  fTime_1=time_1;
  fT_int1=t_int1;fT_ext1=t_ext1;fT_ext2=t_ext2;
...
std::vector< ScanPoint > ReadScanPoints( TFile * fin ){
  ...
  for ( unsigned long long i = 0 ; i < n ; ++i ){
    tt->GetEvent( i );
    result.push_back( ScanPoint( X, Y, Z,Time_1,T_int1,T_ext1,T_ext2, Entry, Entries ) );
  }
  return result;
}
