...
void WaveformAnalysis::ChargeSum( float ped ){
  fitresult->qped = ped;
  float sum = 0.;
...
bool WaveformAnalysis::MonitorCut( float cut ){
  float ped = 0.0;
  int nbins = 20;
...
// bool WaveformAnalysis::PulseLocationCut( int cut ){
...
// is called again
const WaveformFitResult & WaveformAnalysis::get_fitresult( int scanpt, unsigned long long wavenum ){
...
// par[6] = sin phase (rad)
double WaveformAnalysis::pmt0_gaussian(double *x, double *par) {
  double arg=0;
...
// par[2] = sigma
// par[3] = offset
double WaveformAnalysis::pmt1_gaussian(double *x, double *par) {
  double arg=0;
  if(par[2]!=0) arg=(x[0]-par[1])/par[2];
...
double WaveformAnalysis::funcEMG(double *x, double *p){
...
// par[2] = amplitude1
// par[3] = amplitude2
double WaveformAnalysis::pmt2_piecewise(double *x, double *par) {
  if(x[0] < par[0]){
    return par[2];
  }
  else if(x[0] >= par[1]){
...
double WaveformAnalysis::bessel(double *x, double *p){
  //Exponential gaussian used for fitting
...
// void WaveformAnalysis::InitializeFitResult( int wavenum, int nwaves  ) {
//   fitresult->Init();
...
// void WaveformAnalysis::FitWaveform( int wavenum, int nwaves, PTF::PMT pmt) {
...
WaveformAnalysis::WaveformAnalysis( TFile* outfile, Wrapper & wrapper, double errorbar, PTF::PMT & pmt, string config_file, bool savewf ){
  ...
  string ptf_tree_name = "WaveformAnalysis" + std::to_string(pmt.pmt);
  ...
  for (unsigned i = 2; i < wrapper.getNumEntries(); i++) {
    //if ( i>2000 ) continue;
    if( terminal_output ){
      cerr << "WaveformAnalysis scan point " << i << " / " << wrapper.getNumEntries() << "\u001b[34;1m (" << (((double)i)/wrapper.getNumEntries()*100) << "%)\u001b[0m\033[K";
      cerr << "\r";
    }
    else{
      if ( i % 10 == 0 ){
        std::cout << "WaveformAnalysis scan point " << i << " / " << wrapper.getNumEntries() << std::endl;
      }
    }
...
const std::vector< double > WaveformAnalysis::get_bins( char dim ){
  ...
  for(unsigned int iscan=0; iscan<get_nscanpoints(); iscan++){
    ...
    if( dim == 'x' ){
      ...
      cout << "WaveformAnalysis::get_bins Error: input must be x, y or z!" << endl;
      exit( EXIT_FAILURE );
    }
  }
  ...
  if( positions.size() < 3 ){
    cout << "WaveformAnalysis::get_bins Error: fewer than 3 positions a problem for automatic binning!" << endl;
    exit( EXIT_FAILURE );
  }
