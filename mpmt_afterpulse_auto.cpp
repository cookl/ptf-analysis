...
int main( int argc, char* argv[] ) {
  ...
  std::cout << "Performing calculations by pe." <<std::endl;
  for(int i = 0; i < tt->GetEntries(); i++){
    tt->GetEvent(i );
    // std::cout<<i<< ". number of found pulses="<<wf->numPulses << std::endl;
    if(wf->numPulses > 0){
        ...
        if (wf->pulseTimes[i]<afpTimeThreshold){
          ...
          for (int j=1; j<=N; j++) {
            if (j==1){
              if (wfValue<=peRegions[1]) {
                ...
                autoLsrCounter[1] += 1;
                // std::cout<<"Laser at "<<peRegions[j]<<std::endl;
              }
            } else if (j>1 && j<N) {
          ...
          for (int j=1;j<=N;j++) {
            if (autoLsrHappened[j] == true) {
              // std::cout<<"Afterpulse from laser at "<<peRegions[j]<<std::endl;
              autoAfpCounter[j] += 1;
              autoLsrHappened[j] = false;
            }
          }
  ...
  // AFTER PULSES histogram
  // canvas5->cd();
  // histQfrac->SetTitle("Laser pulses");
  // histQfrac->GetXaxis()->SetTitle("Laser pulse height (p.e.)");
  // histQfrac->GetYaxis()->SetTitle("No. of events");
  // // histQfrac->GetXaxis()->SetRange(0,9);
  // histQfrac->Draw();
  // canvas5->SaveAs("../mpmt-frac-252.png");
