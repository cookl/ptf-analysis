...
int main( int argc, char* argv[] ) {
  ...
  char fnmT[1024], fnmQ[1024], fnmQlsr[1024], fnmQafp[1024], fnmAfpRate[1024], fnmSingleAfpRate[1024];
...
int upToWhatPoint=8; // </ choose this after seeing the graphs a first time
...
for (int i=0; i<upToWhatPoint; i++){
  afpGraph->SetPointX(i, peRegions[i+1]);
  afpGraph->SetPointY(i, 100.*afpRates[i+1]);
}
...
for (int i=0; i<upToWhatPoint; i++){
  afpSingleGraph->SetPointX(i, peRegions[i+1]);
  afpSingleGraph->SetPointY(i, 100.0*singleEventAfpRates[i+1]);
}
