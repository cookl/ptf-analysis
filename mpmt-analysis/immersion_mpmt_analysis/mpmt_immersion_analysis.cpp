...
int main( int argc, char* argv[] ) {
	...
	for(int j : myChannels){
		...
		char png_name[100];
		sprintf(png_name,"mpmt_pulse_height%i.png",j);
		c1->SaveAs(png_name);
	}
	...
	for(int j : myChannels){
		...
		char png_name[100];
		sprintf(png_name,"mpmt_pulse_charge%i.png",j);
		c2->SaveAs(png_name);
	}
	...
	//h15->Fit("gaus","","",1.9,17);
	gStyle->SetOptFit(11);
	c13->SaveAs("coincident_channels_2.png");
	...
	//h16->Fit("gaus","","",1.9,17);
	gStyle->SetOptFit(11);
	c14->SaveAs("coincident_channels_4.png");
