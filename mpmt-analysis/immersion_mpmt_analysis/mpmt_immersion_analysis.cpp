// Hassan Elshabasy Immersion mpmt-Analysis Histograms
...
int main( int argc, char* argv[] ) {
	...
	TH1F *h17 = new TH1F("coincidence_charges_ratio_2", "Ratio Coincidence Charges 2 Channels",200,0,30*0.48828125);
	TH1F *h18 = new TH1F("coincidence_charges_ratio_4", "Ratio Coincidence Charges 4 Channels",200,0,200*0.48828125);
	
	int twoNum = 0;
	int fourNum = 0;

	for(int i = 0; i <= 383397; i++){
		std::vector<Pulse> same_event = {};
		...
		for(Pulse pulse : myPulses){ // find pulses with same_events
			if(i == pulse.event){
				same_event.push_back(pulse);
			}
		}
		
		if(same_event.size() < 2){
			continue;
		}
			
		for(Pulse pulse_check : same_event){ // find pulses with reasonable time_difference
